export type BusinessPortalsCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildBusinessPortalsCoreMigrationSql(options: BusinessPortalsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "business_portals_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getBusinessPortalsCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getBusinessPortalsCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildBusinessPortalsCoreRollbackSql(options: BusinessPortalsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "business_portals_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "business_portals_core";
  return [
...
export function getBusinessPortalsCoreLookupIndexName(): string {
  return "business_portals_core_primary_lookup_idx";
}

export function getBusinessPortalsCoreStatusIndexName(): string {
  return "business_portals_core_exception_status_idx";
}
