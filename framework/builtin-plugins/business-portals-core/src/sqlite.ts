export type BusinessPortalsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildBusinessPortalsCoreSqliteMigrationSql(options: BusinessPortalsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "business_portals_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getBusinessPortalsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getBusinessPortalsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildBusinessPortalsCoreSqliteRollbackSql(options: BusinessPortalsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "business_portals_core_");
  return [
...
export function getBusinessPortalsCoreSqliteLookupIndexName(tablePrefix = "business_portals_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getBusinessPortalsCoreSqliteStatusIndexName(tablePrefix = "business_portals_core_"): string {
