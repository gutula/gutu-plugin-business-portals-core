...
describe("business-portals-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "business-portals-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "business-portals-core:demo",
      title: "Business Portals Core Demo",
      ...
      correlationId: "business-portals-core:corr",
      processId: "business-portals-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "business-portals-core:demo",
      exceptionId: "business-portals-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "business-portals-core:demo",
      amendedRecordId: "business-portals-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Business Portals Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("business-portals-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "business-portals-core:demo:amended",
      reversalRecordId: "business-portals-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("business-portals-core:demo:amended:reversal");
